        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
              ...
              {/* ATS Score (kept above content like original) */}
              {atsScore !== null && (
                <div className="mb-6 p-5 rounded-xl bg-gradient-to-br from-teal-50 to-cyan-50 border border-teal-100">
                      ...
                      <div className="relative w-20 h-20">
                        <svg className="transform -rotate-90 w-20 h-20">
                          ...
                          <circle
                            ...
                            className={`transition-all duration-500 ${atsScore >= 80 ? "text-teal-600" : atsScore >= 60 ? "text-yellow-500" : "text-red-500"}`}
                          />
                        </svg>
                        <div className="absolute inset-0 flex items-center justify-center">
                          <span className={`text-2xl font-bold ${atsScore >= 80 ? "text-teal-600" : atsScore >= 60 ? "text-yellow-600" : "text-red-600"}`}>
                            {atsScore}
                          </span>
                        </div>
                      </div>
                  ...
                  <div className="mt-3 pt-3 border-t border-teal-200">
                    <p className={`text-xs font-medium ${atsScore >= 80 ? "text-teal-700" : atsScore >= 60 ? "text-yellow-700" : "text-red-700"}`}>
                      {atsScore >= 80 ? "✓ Excellent ATS compatibility" : atsScore >= 60 ? "⚠ Good, but could be improved" : "⚠ Needs significant improvements for ATS"}
                    </p>
                  </div>
                </div>
              )}
                ...
                {!content ? (
                  <div className="flex flex-col justify-center items-center text-center py-16">
                    ...
                    <p className="text-gray-500 max-w-xs">
                      Upload a resume and click "Review Resume" to get AI-powered feedback and ATS score
                    </p>
                  </div>
                ) : (
                  <div className="px-4 py-3">
                    <Markdown
                      ...
                      components={{
                        h1: ({ children }) => (
                          <h1 className="text-3xl font-bold text-gray-900 mb-6 mt-8 border-b-2 border-gray-200 pb-3">{children}</h1>
                        ),
                        h2: ({ children }) => (
                          <h2 className="text-2xl font-bold text-gray-800 mb-4 mt-6">{children}</h2>
                        ),
                        h3: ({ children }) => (
                          <h3 className="text-xl font-semibold text-gray-700 mb-3 mt-5">{children}</h3>
                        ),
                        h4: ({ children }) => (
                          <h4 className="text-lg font-semibold text-gray-700 mb-2 mt-4">{children}</h4>
                        ),
                        p: ({ children }) => (
                          <p className="text-gray-700 mb-4 leading-7 text-base">{children}</p>
                        ),
                        ...
                        blockquote: ({ children }) => <blockquote className="border-l-4 border-teal-500 pl-4 italic text-gray-600 my-4 bg-teal-50 py-2">{children}</blockquote>,
                      }}
                    >
                      {content}
                    </Markdown>
